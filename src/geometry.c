...
void TE3D_Geometry_ConstructCube(TE3D_Model4f* model, float cubesize, TE3D_Vector3f cubeoffset)
{
	...
	{
		TE3D_VectorIndex2 indices[] = {TE3D_VectorIndex2_N(model->Vectors.count + 0, model->Vectors.count + 1),
	...
	else if (model->Format == TE3D_VECTORFORMAT_TRIANGLES)
	{
		...
		TE3D_VectorIndex3 indices[] = {TE3D_VectorIndex3_N(model->Vectors.count + 0, model->Vectors.count + 1, model->Vectors.count + 2),
