...
/*
Ich habe die Datei mal etwas brauchbarer Struckturiert, ich hoffe es ist alles noch brauchbar.
...
/*
    ...
    Worum handelt es sich bei den nachfolgenden Beschreibungen?
    Ist die Struktur TE3D_Surface nicht schon in einer Headerdatei definiert?
    Ein Semikolon darf innerhalb der funktionsbeschreibung nicht verwendet werden, oder soll dies in den Header?
    Wo kommt die Variable Vectors her?
    Die Funktion printf sollte in der fertigen Version möglichst nicht verwendet werden, da diese die Ausgabe überrscheibt. Scanf sollte nach möglichkeit auch
    nicht umbedingt innerhalb der Bibliothek verwendet werden. (Falls es dennoch nötig ist scanf_s ?)

    Dieser Kommentar kann gerne wieder entfernt werden,
    Frank
